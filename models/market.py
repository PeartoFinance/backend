...
class MarketData(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'changePercent': float(self.change_percent) if self.change_percent is not None else None,
            'volume': self.volume,
            ...
            'previousClose': float(self.previous_close) if self.previous_close is not None else None,
            'dayHigh': float(self.day_high) if self.day_high is not None else None,
            'dayLow': float(self.day_low) if self.day_low is not None else None,
